...
const getApiVersion = () => {
  const version = apiVersionInput.value.trim();
  return version ? version : "1";
};

const getRequestUrl = (path) => {
  const versionedPath = `/${getApiVersion()}${path}`;
  if (proxyToggle.checked) {
    return `/api${versionedPath}`;
  }
  return `${getBaseUrl()}${versionedPath}`;
};
...
const requestPairing = async () => {
  const url = getRequestUrl("/pair/request");
...
const confirmPairing = async () => {
  ...
  const url = getRequestUrl("/pair/grant");
...
const sendKey = async (key) => {
  const url = getRequestUrl("/input/key");
...
const launchApp = async (app) => {
  const url = getRequestUrl("/activities/launch");
...
updateButton.addEventListener("click", () => {
  ...
  addLogEntry(`Connection updated (API v${getApiVersion()})`);
});
